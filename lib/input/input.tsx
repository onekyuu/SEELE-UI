...
const Input: SFC<Props> = (props) => {
    ...
    return (
        <div className={classes(sc(''), inputType, className, size, disabled ?
            'disabled' : '', inputState, inputValue && 'hasInput')}>
            {before && <span className={sc('before')}>{before}</span>}
            <div className={sc('container')}>
                <input
                    className={classes(sc('text'), !labelText ? 'noLabel' : '')}
                    {...rest}
                    ...
                    onKeyDown={onKeyDown}/>
                {labelText && <span
                    className={classes(sc('label'), labelPositionClass)}>{labelText}</span>}
